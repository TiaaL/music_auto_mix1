...
| Field | Purpose |
|---|---|
| `processing_timings_sec` | Per-step wall-clock timing inside `master_loudness_finalize.py`; used to identify slow `loudnorm`, limiter, and de-click stages. |
...
- optionally remove `processing_timings_sec` if no longer profiling finalizer cost
