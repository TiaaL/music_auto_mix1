...
const withVolumeAutomation = args.includes("--with-volume-automation");
const cleanArgs = args.filter((arg) => arg !== "--with-volume-automation");
const [templateId, vocalIn, accompIn, finalOut] = cleanArgs;
...
const templateChains = {
  template_a: {
    vocal: ["c1_gate", "template_a_vocal_proq3", "c1_comp", "sibilance_mono", "vocal_group_fx"],
    accomp: ["template_music_proq3_ab"],
    master: ["template_bus_proq3_ab", "gw_mixcentric_stereo", "master_l2_stereo"],
  },
  template_b: {
    vocal: ["rbass_mono", "f6_rta_mono", "c1_comp", "sibilance_mono", "l1_limiter_mono", "vocal_group_fx"],
    accomp: ["template_music_proq3_ab"],
    master: ["template_bus_proq3_ab", "gw_mixcentric_stereo", "master_l2_stereo"],
  },
  template_c: {
    vocal: ["template_c_vocal_proq3", "vocal_rider_mono", "c1_comp", "oneknob_brighter_mono", "vocal_group_fx"],
...
async function runVolumeAutomationIfRequested(tempDir) {
  ...
  if (proc.status !== 0) {
    throw new Error(`auto_volume_mix.py failed with code ${proc.status}`);
  }
...
try {
  ...
  const vocalAudio = await decodeWav(inputs.vocal);
  const accompAudio = await decodeWav(inputs.accomp);
  ...
  const vocalRendered = await renderChain(chain.vocal, inputs.vocal, tempDir, "vocal", sampleRate, samples);
  const accompRendered = await renderChain(chain.accomp, inputs.accomp, tempDir, "accomp", sampleRate, samples);
  ...
  const masterRendered = await renderChain(chain.master, mixPath, tempDir, "master", sampleRate, samples);
